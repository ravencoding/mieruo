...
String[] localIP = {"192","168","11","0"};
NSLookUpJavaEdition nslookup = new NSLookUpJavaEdition();
PImage img1;
PImage img2;
...
void setup(){
  ...
  //c.setVolumeLimit(4);
  img1 = loadImage("Dropbox-icon.png");
  img2 = loadImage("twitter.png");
...
synchronized void drawNodes() {
  ...
  while(it.hasNext()){
    ...
    //println("ip:" + ip);
    x = int(ip_as_array[2]) * (width-300) / 255; // Scale to applet size
    y = int(ip_as_array[3]) * (height-300) / 255; // Scale to applet size
    x += 50;
    y += 50;
    ...
    if(ip_as_array[0].equals(localIP[0]) && ip_as_array[1].equals(localIP[1])
         && ip_as_array[2].equals(localIP[2])){
      ...
      String domain = nslookup.execute(ip);
      //text(ip,x,y);
      text(domain,x,y);
      //fill(color(100,100,100,50));
      fill(color(int(ip_as_array[0]),int(ip_as_array[1]),int(ip_as_array[2]),50));
      ellipse(x, y, d + 20, d + 20);
      
      
      if(ip.equals("108.160.162.99")){
        image(img1, x - img1.width/2, y - img1.height/2);
      }else if(ip.equals("199.59.149.201")){
        image(img2, x - img2.width/2, y - img2.height/2);
      }
...
synchronized void packetEvent(CarnivorePacket packet){
  ...
  println("data:" + packet.flagsToString());
  println("srcport:" + packet.senderPort);
  println("destport:" + packet.receiverPort);
